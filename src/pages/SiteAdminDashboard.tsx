...
export default function SiteAdminDashboard() {
  ...
  const loadPods = async () => {
    try {
      ...
      const response = await fetch(`http://productionv36.qikpod.com:8989/pods/?location_id=${currentLocationId}&pod_mode=adhoc`, {
        method: 'GET',
        headers: {
  ...
  const loadLocationUsers = async () => {
    try {
      const authToken = localStorage.getItem('auth_token');
      const response = await fetch(`http://productionv36.qikpod.com:8989/users/locations/?location_id=${currentLocationId}`, {
        method: 'GET',
        headers: {
          'accept': 'application/json',
          'Authorization': `Bearer ${authToken}`
  ...
  const loadHistory = async () => {
    try {
      const authToken = localStorage.getItem('auth_token');
      const response = await fetch(`http://productionv36.qikpod.com:8989/adhoc/reservations/?location_id=${currentLocationId}`, {
        method: 'GET',
        headers: {
          'accept': 'application/json',
          'Authorization': `Bearer ${authToken}`
